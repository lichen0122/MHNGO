...
using System.Text;
using MHNGO.Services;
using MHNGO.Services.Location;
using MHNGO.Services.Mount;
using iMobileDevice;
...
namespace MHNGO {
    ...
    {
        private const double R = 6371e3;  // Earth radius in meters
        private const int step_range = 25;

        public static List<(double latitude, double longitude)> GetPoints(double lat1, double lon1, double lat2, double lon2)
        {

            double distance = HaversineDistance(lat1, lon1, lat2, lon2);
            int segments = (int)(distance / step_range);
            List<(double latitude, double longitude)> points = new List<(double latitude, double longitude)>();

            if(distance < step_range)
            {
                // Do something specific, e.g.:
                return new List<(double latitude, double longitude)> { (lat2, lon2) };  // Return only the end point
            }
            ...
            for (int i = 0; i <= segments; i++)
            {
                ...
                points.Add(IntermediatePoint(lat1, lon1, lat2, lon2, fraction));
            }
        ...
        private static (double latitude, double longitude) IntermediatePoint(double lat1, double lon1, double lat2, double lon2, double fraction)
        {
            lat1 = ToRadians(lat1);
            lon1 = ToRadians(lon1);
            lat2 = ToRadians(lat2);
            ...
            double d = HaversineDistance(ToDegrees(lat1), ToDegrees(lon1), ToDegrees(lat2), ToDegrees(lon2));

            double A = Math.Sin((1 - fraction) * d / R) / Math.Sin(d / R);
            double B = Math.Sin(fraction * d / R) / Math.Sin(d / R);
    ...
    class DeviceInformation {
        private static PointLatLng? last_target = new PointLatLng(-1, -1);
        ...
        public void SetLocation(PointLatLng? target) {
            ...
            if (int.Parse(((string)Properties["ProductVersion"]).Split('.')[0]) >= 17) {
                ...
                //Console.WriteLine($"{target.Value.Lat}, {target.Value.Lng}");
                /*if (last_target.Value.Lat == -1 && last_target.Value.Lng == -1)
                {
                    new DtSimulateLocation(this).SetLocation(target);
                    last_target = new PointLatLng(target.Value.Lat, target.Value.Lng);
                }
                else
                {
                    List<(double latitude, double longitude)> points = GPSUtility.GetPoints(last_target.Value.Lat, last_target.Value.Lng, target.Value.Lat, target.Value.Lng);
                    Console.WriteLine("開始移動");
                    foreach (var point in points)
                    {
                        PointLatLng? cur_target = new PointLatLng(point.latitude, point.longitude);
                        Console.WriteLine($"{point.latitude}, {point.longitude}");
                        new DtSimulateLocation(this).SetLocation(cur_target);
